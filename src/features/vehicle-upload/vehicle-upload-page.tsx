...
import { UploadLivePreview } from "@/features/vehicle-upload/components/upload-live-preview";
...
function VehicleUploadContent() {
  const { lastSavedAt, isSaving, isSuccessStep } = useUploadWizard();
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <Link
              ...
              className="mb-4 inline-flex items-center gap-1.5 text-[length:var(--text-body-sm)] font-medium text-[var(--color-muted-foreground)] motion-nav hover:text-[var(--color-primary)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-focus-ring)] rounded-[var(--radius-sm)]"
            >
            ...
            <p className="mt-2 max-w-2xl text-[length:var(--text-body-sm)] leading-[var(--leading-relaxed)] text-[var(--color-muted-foreground)] lg:text-[length:var(--text-body-md)]">
              One upload powers marketplace, website, search, AI, and marketing — your single source of truth.
            </p>
          </div>
          {lastSavedAt && (
            <div
