...
function growthRate(current: number, previous: number): string {
  if (previous <= 0) return "No data yet";
  const value = Math.round(((current - previous) / previous) * 100);
  return `${value >= 0 ? "+" : ""}${value}%`;
}

function stream(id: string, label: string, status: "live" | "unavailable", value: string, detail: string) {
  return {
    id,
    stream: label,
    status,
...
export async function getRevenueCentreWorkspaceData(sectionId: RevenueCentreSectionId): Promise<RevenueCentreWorkspaceData> {
  ...
  for (const dealer of dealerships) {
    const pkg = dealer.subscriptionPackage ?? "No data yet";
  ...
  const subscriptions: SubscriptionRow[] = [...packageCounts.entries()]
    .map(([packageId, dealerCount]) => {
      const availability: SubscriptionRow["availability"] = packageId === "No data yet" ? "unavailable" : "live";

      return {
        packageId,
        packageLabel: packageId,
        dealerCount,
        renewals: "No data yet",
        expiries: "No data yet",
        upgrades: "No data yet",
        downgrades: "No data yet",
        cancelled: "No data yet",
        trials: packageId.toLowerCase().includes("trial") ? dealerCount : 0,
        revenueByPackage: "No data yet",
        availability,
      };
    })
  ...
  const revenueStreams = [
    stream(
      "subscriptions",
      "Subscriptions",
      subscriptions.some((item) => item.availability === "live") ? "live" : "unavailable",
    ...
    stream(
      "finance-commission",
      "Finance Commission",
      financeRequests > 0 ? "live" : "unavailable",
      financeRequests.toLocaleString("en-ZA"),
      "Live finance request volume from enquiries. Commission settlement is No data yet.",
    ),
    stream("insurance-commission", "Insurance Commission", "unavailable", "No data yet", "Insurance commission feeds are not connected yet."),
    stream("warranty-commission", "Warranty Commission", "unavailable", "No data yet", "Warranty commission feeds are not connected yet."),
    stream("partner-revenue", "Partner Revenue", "unavailable", "No data yet", "Partner payout and revenue feeds are not connected yet."),
    stream("affiliate-revenue", "Affiliate Revenue", "unavailable", "No data yet", "Affiliate attribution and payout feeds are not connected yet."),
    stream("marketplace-services", "Marketplace Services", "unavailable", "No data yet", "Marketplace services monetization ledger is not connected yet."),
  ];
  ...
  const dealerRevenue = dealerships.map((dealer) => {
    ...
    return {
      ...
      packageLabel: dealer.subscriptionPackage ?? "No data yet",
      spend: "No data yet",
      invoices: "No data yet",
      outstanding: "No data yet",
      revenueTrend: performance?.conversion ?? "No data yet",
      growth: "No data yet",
      health: health ? `${health.healthScore} (${health.risk})` : "No data yet",
    };
  });
  ...
  return {
    ...
    sectionId,
    summaryCards: [
      { id: "mrr", label: "Monthly Recurring Revenue", value: "No data yet", detail: "Subscription billing amounts are not connected yet.", availability: "unavailable" },
      { id: "arr", label: "Annual Recurring Revenue", value: "No data yet", detail: "Annualized recurring ledger is not connected yet.", availability: "unavailable" },
      ...
      { id: "revenue-today", label: "Revenue Today", value: "No data yet", detail: "Payment capture data is not connected yet.", availability: "unavailable" },
      { id: "revenue-month", label: "Revenue This Month", value: "No data yet", detail: "Monthly recognized revenue ledger is not connected yet.", availability: "unavailable" },
      { id: "revenue-year", label: "Revenue This Year", value: "No data yet", detail: "Annual recognized revenue ledger is not connected yet.", availability: "unavailable" },
      { id: "arpd", label: "Average Revenue Per Dealer", value: "No data yet", detail: "Requires live dealer revenue ledger integration.", availability: "unavailable" },
      { id: "clv", label: "Customer Lifetime Value", value: "No data yet", detail: "Requires historical billing and retention data.", availability: "unavailable" },
      { id: "outstanding", label: "Outstanding Revenue", value: "No data yet", detail: "Invoice and collections systems are not connected yet.", availability: "unavailable" },
      { id: "forecast", label: "Revenue Forecast", value: "No data yet", detail: "Forecasting framework is ready; financial projections await live feeds.", availability: "unavailable" },
      { id: "growth", label: "Platform Growth", value: growthRate(newDealersThisMonth, newDealersPreviousMonth), detail: "Live dealer onboarding growth trend month-over-month.", availability: newDealersPreviousMonth > 0 ? "live" : "unavailable" },
    ],
    subscriptions,
    revenueStreams,
    dealerRevenue,
    outstandingRevenue: {
      outstandingInvoices: "No data yet",
      overdueSubscriptions: "No data yet",
      pendingPayments: "No data yet",
      collections: "No data yet",
      paymentStatus: "No data yet",
      availability: "unavailable",
    },
    forecasting: [
      { id: "monthly", label: "Monthly Revenue", value: "No data yet", detail: "Awaiting live recurring and transactional revenue feeds.", availability: "unavailable" },
      { id: "quarterly", label: "Quarterly Revenue", value: "No data yet", detail: "Awaiting live recurring and transactional revenue feeds.", availability: "unavailable" },
      { id: "annual", label: "Annual Revenue", value: "No data yet", detail: "Awaiting live recurring and transactional revenue feeds.", availability: "unavailable" },
      { id: "growth", label: "Growth", value: growthRate(newDealersThisMonth, newDealersPreviousMonth), detail: "Dealer onboarding growth reused as interim expansion indicator.", availability: newDealersPreviousMonth > 0 ? "live" : "unavailable" },
      { id: "churn", label: "Churn", value: "No data yet", detail: "Subscription churn feed not connected.", availability: "unavailable" },
    ...
    trends: [
      ...
      { id: "finance-demand", label: "Finance Demand", value: financeRequests.toLocaleString("en-ZA"), detail: "Live finance request volume through applications/enquiries.", availability: financeRequests > 0 ? "live" : "unavailable" },
    ],
    reports: [
      ...
      { id: "collections-report", name: "Outstanding Collections Report", status: "unavailable", detail: "Requires invoice/payment status integrations." },
      { id: "commissions-report", name: "Commission Streams Report", status: "unavailable", detail: "Requires finance/insurance/warranty/partner commission settlement feeds." },
    ],
    timeline,
    audit,
    sourceReadiness: [
      ...
      { id: "billing-ledger", label: "Billing, Invoices, Payments, Commissions", mode: "unavailable", detail: "No live billing/collections ledger is implemented yet." },
    ],
  };
}
