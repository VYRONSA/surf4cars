-- SFC-108 Onboarding completion atomicity and idempotency hardening.
-- Keeps existing entities and contracts while ensuring a single transactional completion boundary.

create or replace function public.complete_dealer_onboarding_atomic(p_payload jsonb)
returns jsonb
language plpgsql
security invoker
as $$
